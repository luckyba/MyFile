...
//    }
    ...
    private void setPassword(String strPassword) {
        ...
        if (passwordLength == 3) {
            if (tempPassword.equals(preferManager.getPassword())) {
                ...
                Toast.makeText(MyApplication.getInstance().getApplicationContext(), "Wrong password", Toast.LENGTH_SHORT).show();
                tempPassword = "";
                txtPassword.setText("");
                passwordLength = 0;
                pswArray.clear();
    ...
    protected void onResume() {
        super.onResume();
        MyApplication.getInstance().trackScreenView("ScreenLock screen");
    }
}
